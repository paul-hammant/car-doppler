...
app.get('/test-mounter', (req, res) => {
    const testMounterPath = path_1.default.resolve(__dirname, 'test-mounter.html');
    if (fs_1.default.existsSync(testMounterPath)) {
...
app.get('/render-component-fragment/:componentName', async (req, res) => {
    ...
    if (react_1.default.isValidElement(componentElementOrError)) {
        ...
        res.status(500).send(`<div>Error rendering component: ${componentElementOrError.error}</div>`);
    }
});
app.listen(PORT, () => {
